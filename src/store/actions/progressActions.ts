...
import {userProgressToJson, userProgressFromJson} from '../../models/UserProgress';
...
export const initializeProgress = (userId: string, variant: string) => async (dispatch: any) => {
  dispatch({type: PROGRESS_INITIALIZE});
  try {
    const localProgress = await StorageService.getProgress(userId);
    if (localProgress) {
      dispatch({type: PROGRESS_LOAD_SUCCESS, payload: localProgress});
    }
    await loadProgress(userId, variant)(dispatch);
  } catch (error: any) {
    dispatch({type: PROGRESS_LOAD_FAILURE, payload: error.message});
  }
};

export const loadProgress = (userId: string, variant: string) => async (dispatch: any) => {
  dispatch({type: PROGRESS_LOAD_START});
  try {
    const doc = await FirebaseService.getFirestore().collection('progress').doc(userId).get();

    let progress: UserProgress;
    if (doc.exists) {
      progress = userProgressFromJson(doc.data()!);
    } else {
      progress = {
        userId,
        variant,
      ...
      await saveProgress(progress)(dispatch);
    }
...
export const updateLevelProgress = (level: LearningLevel, levelProgress: LevelProgress) => async (dispatch: any, getState: any) => {
  ...
  try {
    ...
    await saveProgress(updatedProgress)(dispatch);
...
const saveProgress = (progress: UserProgress) => async (dispatch: any) => {
  try {
    await FirebaseService.getFirestore()
      .collection('progress')
      .doc(progress.userId)
      .set(userProgressToJson(progress), {merge: true});
  } catch (error) {
