...
export function PrototypeVariantProvider({ children }: { children: ReactNode }) {
  ...
  // The URL is a browser-only value, so the server render and the hydration
  // pass both see 'A' and the markup agrees by construction. Reading it here
  // rather than from an effect also means the requested variant is on screen
  // for the first paint instead of the second.
  const fromUrl = useBrowserValue(readInitialVariant, 'A' as VariantKey);
  const [override, setOverride] = useState<VariantKey | null>(null);
  const variant = override ?? (isDev ? fromUrl : 'A');
  const setVariant = setOverride;
  ...
  // Steps from the variant actually on screen rather than from the override,
  // so arrowing away from a URL-set variant continues from that variant
  // instead of restarting at A.
  const cycle = useCallback(
    (step: number) => {
      change(ORDER[(ORDER.indexOf(variant) + step + ORDER.length) % ORDER.length]);
    },
    [variant, change],
  );
