...
import { isWinningHandWithMelds } from './winDetection';
...
function handleSelfDrawnWin(state: GameState, playerIndex: number): GameState | null {
  ...
  const player = state.players[playerIndex];
  if (!isWinningHandWithMelds(player.hand, player.melds)) return null;
  ...
  if (matchingInHand.length >= 1) {
    ...
    if (pungMeldIndex !== -1) {
      ...
      for (let i = 0; i < state.players.length; i++) {
        ...
        if (isWinningHandWithMelds(otherHand, state.players[i].melds)) {
...
function handleClaim(
  ...
  if (claimType === 'win') {
    ...
    if (!isWinningHandWithMelds(handWithTile, player.melds)) return null;
  } else {
